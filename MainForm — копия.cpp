...
void __fastcall TMainForm::FormCreate(TObject *Sender) {
    ...
    if (!LoadStandardLibrary()) {
        // Если не удалось загрузить из DLL, создаем локальную библиотеку
        if (!CreateLocalLibrary()) {
            Application->MessageBox(L"Не удалось инициализировать библиотеку элементов.\nПриложение будет закрыто.", 
...
bool TMainForm::CreateLocalLibrary() {
    try {
        ...
        ShowMessage("Ошибка создания локальной библиотеки: " + e.Message);
        return false;
    }
    catch (...) {
        ShowMessage("Неизвестная ошибка при создании локальной библиотеки");
        return false;
    }
}
...
bool TMainForm::LoadStandardLibrary() {
    ...
    if (!FileExists(dllPath)) {
        String errorMsg = L"DLL файл не найден по пути:\n" + dllPath + 
                         L"\n\nБудет создана локальная библиотека элементов.";
        Application->MessageBox(errorMsg.w_str(), L"Файл не найден", MB_OK | MB_ICONWARNING);
        return false;
    }
    ...
    if (!FStandardLibraryHandle) {
        ...
        String errorMsg = L"Ошибка загрузки DLL. Код ошибки: " + IntToStr(static_cast<int>(errorCode)) +
                         L"\n\nБудет создана локальная библиотека элементов.";
        Application->MessageBox(errorMsg.w_str(), L"Ошибка загрузки DLL", MB_OK | MB_ICONWARNING);
        return false;
    }
    ...
    if (!registerFunc) {
        String errorMsg = L"Функция RegisterStandardLibrary не найдена в DLL.\n\n" 
                         L"Будет создана локальная библиотека элементов.";
        Application->MessageBox(errorMsg.w_str(), L"Ошибка поиска функции", MB_OK | MB_ICONWARNING);
    ...
    try {
        ...
        if (!result) {
            Application->MessageBox(L"Функция RegisterStandardLibrary вернула false.\n" 
                                   L"Будет создана локальная библиотека элементов.", 
                                   L"Ошибка регистрации", MB_OK | MB_ICONWARNING);
            return false;
        }
        return true;
    }
        ...
        String errorMsg = L"Исключение при вызове RegisterStandardLibrary: " + e.Message +
                         L"\n\nБудет создана локальная библиотека элементов.";
        Application->MessageBox(errorMsg.w_str(), L"Ошибка выполнения", MB_OK | MB_ICONWARNING);
        return false;
    }
    catch (...) {
        Application->MessageBox(L"Неизвестное исключение при вызове RegisterStandardLibrary.\n" 
                               L"Будет создана локальная библиотека элементов.", 
                               L"Ошибка выполнения", MB_OK | MB_ICONWARNING);
        return false;
    }
}
...
TCircuitElement* TMainForm::CreateElementByType(const String& ElementDesc, int X, int Y) {
    if (!LibraryManager || !LibraryManager->GetCurrentLibrary()) {
        ShowMessage("Ошибка: библиотека не инициализирована");
        return nullptr;
    }
    
    if (!LibraryManager->GetCurrentLibrary()->HasElement(ElementDesc)) {
        ShowMessage("Ошибка: элемент '" + ElementDesc + "' не найден в библиотеке");
        return nullptr;
    }
    
    TCircuitElement* element = nullptr;
    try {
        element = LibraryManager->CreateElementFromCurrent(ElementDesc, FNextElementId, X, Y);
        if (element) {
